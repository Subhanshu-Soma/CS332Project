...
log_filename = f"scheduler_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
log_file = None
    ...
    while not shutdown_flag.is_set():
        ...
        if current:
            ...
            try:
                ...
                log_message(f"[PROCESS START] Process {pid} started with burst time {wait}")
                
                
                time.sleep(int(wait))
                
                
                log_message(f"[PROCESS COMPLETE] Process {pid} finished after {wait} seconds")
                running = ""
    ...
    while True:
        try:
            ...
            if s == "exit":
                shutdown_flag.set()
                print("Shutting down...")
                log_message("[SHELL] User initiated shutdown")
                break
            elif s == "list":
                with queue_lock:
                ...
                for msg in messages:
                    if msg:  # Skip empty messages
                        ...
                        if msg == "END":
                            ...
                            # Log receiving process information
                            log_message(f"[RECEIVER] Received process: {msg}")
            
        except ConnectionResetError:
            log_message("[RECEIVER] Connection was reset by the server")
